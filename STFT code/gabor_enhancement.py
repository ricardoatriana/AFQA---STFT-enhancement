...
class GaborEnhancement:
    ...
    def ridge_orient(self, normed_img):
        ...
        coherence_m = coherence_m * (denom > 0.001)  # Ensure coherence is meaningful

        # Compute Rao's angular coherence measure
        S_x = cv2.GaussianBlur(np.cos(2 * orient_image), (ksize, ksize), self.orient_blocksigma)
        S_y = cv2.GaussianBlur(np.sin(2 * orient_image), (ksize, ksize), self.orient_blocksigma)
       
        # Compute N locally (sum inside Gaussian filter)
        N = cv2.GaussianBlur(np.ones_like(orient_image), (ksize, ksize), self.orient_blocksigma)

        coherence_rao = np.sqrt(S_x**2 + S_y**2) / (N + EPS)  # Rao's coherence measure
        
        return orient_image, reliability, coherence_m, coherence_rao
    ...
    def enhance(self, img, extended_data=False):
        ...
        orient_image, reliability, coherence_m, coherence_rao = self.ridge_orient(normed_image)       # compute orientation image
